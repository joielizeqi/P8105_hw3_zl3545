...
In the categories of "less than high school level" and "more than high school 
level", there are slightly more women than men. But there are significantly 
more men than women with a high school equivalent education level. 
...
mims |> 
  ...
  ylab("Age")
```
In the boxplots above, we can see that in the education category "high school equivalent", the median age for women is around 60 and the median age for men 
is around 50; in the education category "less than high school", the median ages 
for both men and women are around 60; in the education category "more than high 
school", the median age for both men and women are around 40. From this 
observation, we note that younger participants tend to have a higher education 
level. In addition, the age distributions of men and women in the "high school equivalent" have a more drastic difference than those in the other two categories.


## Aggregate MIMS data and plot
```{r aggregate_plot, message = FALSE}
aggr_mims = 
  mims |> 
...
aggr_mims |> 
  ...
  xlab("Time")
```
From the plot above, we can see that overall, the median MIMS activity for men 
and women are roughly in the same range. For people with a "more than high school" 
education level, the median MIMS activities for some women are higher at 8:30am in
the morning (at around 500th minute), and the median MIMS activities for some men 
are higher at 8:30pm at night (at around 1250th minute). Besides this, overall, 
the trend lines and scattered points look roughly similar across education levels.
...
mean_mims |> 
  ...
  xlab("Time")
```
From the plot above, we can see that overall, the mean MIMS activity for men 
and women roughly follow the same trend as the median MIMS plot above.

# Problem 3
## Import data
```{r import_data_p3, message = FALSE}
...
```
The resulting dataset contains information about `r nrow(citi)-1` CitiBike rides 
in January 2020, January 2024, July 2020, and July 2024. The variables that describes these rides are `r colnames(citi)`.
...
knitr::kable(member_casual,
             ...
             Years and Months")
```
From the table above, we can see that in general, more there are more rides in 
July than in January. In addition, there more a lot more users in 2024 than in 
2020, and more riders became members in 2024.
...
median_duration |> 
  ...
  xlab("Weekdays")
```
From the side-by-side bar plot above, we can see that in general, the median 
ride duration in July are longer than those in January. This is possibly 
because riders want to minimize their time outdoor in the cold weather in 
January. The effect of year and day of the week are less significant, but we can 
note that in January 2020, ride durations during weekdays are shorter than 
that in the same month in 2024; likewise, in January 2024, ride durations during weekends are shorter than that in the same month in 2020.
...
bike_type |> 
  ...
  xlab("Month")
```
From the boxplots above, we can see the the distributions are very clustered 
around the medians, which are all around 15. There are many outliers at the top 
of all of boxplots, meaning that some users prefer to ride CitiBike for a much 
longer duration than typical CitiBike users. We can also note that casual bikers 
tend to have longer rides than members.
